---
title: "Nociones básicas de geoestadística y dependencia espacial"
author: "Joseba Iribarren (joseba.iribarren@alumnos.uach.cl )"
date: "2025-03-14"
output: 
  html_document:
    toc: true
...
```{r librerias a usar, warning=FALSE, include=FALSE}
require(pacman)
...
### 2) Visualizar capas
(parece que hay una dificultad con la ubicación de los titulos)
```{r ver coberturas}
map1 <- ggplot() +
  geom_sf(data = losRios, fill = 'white') +
  geom_sf(data = aleatorios, color = 'green') +
  # ggtitle(title = 'Indiferencia') + # by horo
  theme_void()

map2 <- ggplot() +
  geom_sf(data = losRios, fill = 'white') +
  geom_sf(data = agrupados, color = 'yellow') +
  # ggtitle(title = 'Atracción') +
  theme_void()

map3 <- ggplot() +
  geom_sf(data = losRios, fill = 'white') +
  geom_sf(data = uniforme, color = 'red') +
  # ggtitle(title = 'Repulsión') +
  theme_void()
...
Hasta el momento solo generamos un analisis con la posición espacial de los puntos, pero esta vez queremos ver como se aplica la ley de Tobler, a traves de un autocorrelograma. Para esto necesitamos valores, los cuales extraeremos de una capa de contenido de arcilla para la region de Los Ríos a una profundidad de 5 a 15 cm.
...
Calculamos la autocovarianza espacial como la suma de los productos de las desviaciones de cada par de puntos, respecto a la media global obtenida en todo el paisaje para esa variable.\

Si hacemos este analisis para distintas distancias podemos tener una intuición de a que distancia se pierde la correlación espacial.\
