...
El semivariograma es una herramient ocupada para ver como cambia la variabilidad espacial con la distancia. Y se calcula mediante la mitad de la diferencia cuadrática media entre puntos separados por una distancia.

![](imagenes/formula.png)
![](imagenes/esquema.png)
*Nugget:* es el valor de la semivarianza en h = 0 (es decir, en la intersección con el eje y). Aunque en teoría debería ser cero, la intersección suele ser mayor que cero en la práctica, debido a la presencia de heterogeneidad de microescala que no hemos medido (nuestra granularidad de muestreo es demasiado grande) o debido al muestreo o errores de medición resultantes de diferencias entre observadores (sesgo del observador), errores de instrumentación, etc. (Fortin y Dale, 2005).\

*Rango:* es la distancia (h) a lo largo de la cual la variable medida está autocorrelacionada espacialmente. El rango se identifica por el punto donde los valores de semivarianza comienzan a estabilizarse (es decir, el Sill).\

*Sill:* se refiere a la parte del variograma donde la semivarianza se estabiliza y forma una meseta. En este dominio, la distancia ya no afecta la estructura espacial de los datos (es decir, los datos son espacialmente independientes). Sin embargo, no todos los patrones espaciales tienen un Sill o umbral.\
...
2)  Adicionalmente, sin el uso de mapas, y en base a los analisis anteriores, describa cómo se distribuyen los puntos a distintas escalas y en qué punto, aproximadamente se pierde la autocorrelación espacial.\
